...
- 2026-06-03 — M1 patch: `apps/accounts/services/accounts.py` must NEVER import from `core.exceptions`. That module is loaded during DRF settings init (via `authentication.py`) and `core.exceptions` imports `rest_framework.views`, creating a circular import. Validation for accounts service inputs lives in the serializer layer only. Any other service imported by an auth class has the same constraint. (M1 patch)
- 2026-06-03 — M1 patch: email/OTP name-collection flow — `PATCH /api/v1/auth/me {display_name}` populates the name for new email/OTP users (Firebase supplies no `name` claim for non-social providers). Flutter routing: after `POST /register`, if `created=true` AND `display_name==""` → show name screen → `PATCH /auth/me` → then check `has_profile` for questionnaire routing. Google users are unaffected (name arrives in token). Returning email users are unaffected (name already in DB). (M1 patch)
