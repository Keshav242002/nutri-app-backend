...
def build_ingredient_prompt(
    ...
    system_content = (
        ...
        f"\n"
        f"IMPORTANT RULES:\n"
        f"1. ONLY use ingredient names from this approved list: {available_str}\n"
        f"2. ingredient_name must EXACTLY match a name from the list above.\n"
        f"3. Return ONLY valid JSON matching the schema below. "
        f"No markdown, no prose, no code fences.\n"
        f"4. quantity_grams must be between 1 and 5000.\n"
        f"5. Servings must be between 1 and 12.\n"
        f"\n"
